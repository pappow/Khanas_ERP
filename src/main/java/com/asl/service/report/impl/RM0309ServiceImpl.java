...
public class RM0309ServiceImpl extends AbstractReportService {
	...
	private List<FormFieldBuilder> generateFields() {
		...
		List<Xcodes> SupgroupList = xcodesService.findByXtype(CodeType.SUPPLIER_GROUP.getCode(), Boolean.TRUE);
		List<DropdownOption> group = new ArrayList<>();
		group.add(new DropdownOption("", "-- Select --"));
		SupgroupList.stream().forEach(x -> group.add(new DropdownOption(x.getXcode(), x.getXcode())));
		...
		// Payment By
		fieldsList.add(FormFieldBuilder.generateSearchField(6, "Payment By", "search/report/xstaff", "", false));
